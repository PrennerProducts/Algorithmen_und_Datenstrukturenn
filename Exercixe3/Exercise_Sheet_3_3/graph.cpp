...
int Graph::getEdgeWeight(std::string sourceVertex, std::string targetVertex) {
    int mySource = this->resolveVertex(sourceVertex);
    int myTarget = this->resolveVertex(targetVertex);
    return this->adjacencyMatrix[mySource][myTarget];
}

void Graph::setEdgeWeight(std::string sourceVertex, std::string targetVertex, int weight) {
    Edge addEdge(sourceVertex, targetVertex, weight);
    this->insertEdge(addEdge);
}
