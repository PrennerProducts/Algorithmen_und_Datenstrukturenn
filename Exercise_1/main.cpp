//
// Created by McRebel on 26.05.2022.
//



#include <iostream>
#include "Ware.h"
#include "Sortiment.h"
#include "random"
#include "ctime"



int main() {
    srand(time(0));

    std::cout << "Hello from main" << std::endl;

    Sortiment sortiment;

    Ware Banane("Banana", 11, 19, 6.55, 25.64);
    Ware Apfel("Apfel", 8, 88, 1.57, 4.56);
    Ware Birne("Birne", 48, 55, 8.07, 7.16);
    Ware Avocado("Avocado", 4, 78, 9.57, 15.63);
    Ware Trauben("Trauben", 5, 654, 7.75, 10.46);
    Ware Zitronen("Zitronen", 78, 50, 4.55, 7.46);
    Ware Melonen("Melonen", 477, 4, 1.45, 2.36);
    Ware Nuesse("Nuesse", 38, 45, 4.45, 8.65);
    Ware Datteln("Datteln", 19, 8, 1.81, 2.96);
    Ware Feigen("Feigen", 10, 33, 1.70, 2.46);


    sortiment.addWare(&Banane);
    sortiment.addWare(&Apfel);
    sortiment.addWare(&Birne);
    sortiment.addWare(&Avocado);
    sortiment.addWare(&Trauben);
    sortiment.addWare(&Zitronen);
    sortiment.addWare(&Melonen);
    sortiment.addWare(&Nuesse);
    sortiment.addWare(&Datteln);
    sortiment.addWare(&Feigen);

    sortiment.printArray(sortiment.waren, 10, "Waren: ");

    sortiment.sort(5);

    sortiment.printArray(sortiment.waren, 10, "Waren: ");



    return 0;
}