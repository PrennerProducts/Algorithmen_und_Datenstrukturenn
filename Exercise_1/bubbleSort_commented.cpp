...
void bubbleSort(int* array, int array_size, bool asc) {     // Funktion mit einem int array , und der array_size, der bool Wert asc wird in der Funktion nicht verwendet daher kann er weggelassen werden!
    ...
    do {
        ...
        for(int i=0; i<array_size-1;i++){                   // Schleife mit array_size durchläufen. Schlüsselstelle!
    ...
    } while (swapped == true);                              // Schlüsselstelle! Im worst case (Wenn das Array genau umgekerht Sortiert ist) muss für jede Iteration in der for-Schleife (Anzahl = n), die while-Schleife
